...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Local entrypoint for the normalized-document full flow. Accepts document.v1 JSON plus the source PDF.",
    )
    parser.add_argument(
        "input_dir",
        nargs="?",
    ...
    parser.add_argument(
        "--math-mode",
        type=str,
        default="direct_typst",
        choices=["placeholder", "direct_typst"],
        help="Math translation mode. Default is direct_typst.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="typst",
        choices=["auto", "overlay", "typst", "dual"],
        help="Rendering mode for translated pages. auto chooses between typst overlay and typst background.",
    )
    parser.add_argument(
        "--compile-workers",
        type=int,
