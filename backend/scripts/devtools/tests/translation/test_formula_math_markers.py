...
def test_restored_formula_tokens_are_wrapped_as_inline_math() -> None:
    ...
    restored = restore_protected_tokens("calcium peroxide<f1-17a/>releases", formula_map)
    assert restored == r"calcium peroxide$(\mathrm{CaO}_2)$releases"
...
def test_reprotect_restored_formula_accepts_inline_math_marker() -> None:
    ...
    protected = re_protect_restored_formulas(r"calcium peroxide$(\mathrm{CaO}_2)$releases", formula_map)
    assert protected == "calcium peroxide<f1-17a/>releases"
...
def test_reprotect_restored_formula_does_not_mutate_existing_typed_tokens() -> None:
    ...
    protected = re_protect_restored_formulas(
        "However, research shows that these traditional methods are not suitable for characterizing semiconductors with localized electronic states<f1-9a9/>or<f2-797/>orbitals).",
        formula_map,
    )
    assert protected == "However, research shows that these traditional methods are not suitable for characterizing semiconductors with localized electronic states<f1-9a9/>or<f2-797/>orbitals)."
...
def test_typst_markdown_supports_typed_formula_placeholders() -> None:
    formula_map = [{"placeholder": "<f1-17a/>", "formula_text": r"(\mathrm{CaO}_2)"}]
    markdown = build_markdown_from_parts("calcium peroxide<f1-17a/>releases", formula_map)
    assert markdown == r"calcium peroxide $(\mathrm{CaO}_2)$ releases"


def test_typst_markdown_supports_direct_math_text_without_formula_map() -> None:
    markdown = build_markdown_from_direct_text(r"The transition matrix Q_t indicates, and x_t in {0,1}^K.")
...
def test_render_markdown_uses_direct_typst_path_for_item() -> None:
    ...
    markdown = build_item_render_markdown(item, r"integral$\int f(x) dx$value", [])
...
def test_render_markdown_uses_formula_map_for_placeholder_mode() -> None:
    ...
    markdown = build_render_markdown("Hello<f1-17a/>, next step", formula_map, math_mode="placeholder")
...
def test_typst_markdown_direct_typst_conservative_mode_does_not_guess_plain_scripts() -> None:
    markdown = build_markdown_from_direct_text(
        r"The transition matrix Q_t indicates, and x_t in {0,1}^K.",
...
def test_typst_markdown_direct_typst_conservative_mode_keeps_raw_latex_text() -> None:
    markdown = build_markdown_from_direct_text(
        r"Ions are \left[ NTf _ { 2 } \right] ，and form \mathrm { Co(IV) } species.",
        aggressive_math_promotion=False,
    )
...
def test_typst_markdown_direct_typst_keeps_existing_inline_math_latex() -> None:
    markdown = build_markdown_from_direct_text(
        r"Observed that $\mathrm{Ph(i-PrO)SiH_2}$ (6) was consumed faster than other silanes.",
        aggressive_math_promotion=False,
...
def test_typst_markdown_escapes_literal_double_asterisk_in_plain_text() -> None:
    markdown = build_markdown_from_direct_text(
        r"Calculated using 6-310** basis set and corresponding optimized geometry.",
        aggressive_math_promotion=False,
    )
...
def test_direct_typst_passthrough_escapes_literal_double_asterisk_outside_math() -> None:
    markdown = build_direct_typst_passthrough_text(
        r"Using 6-310** basis set, and keeping $E=mc^2$ unchanged."
    )
    assert r"6-310\*\*" in markdown
...
def test_build_markdown_from_parts_direct_typst_passthrough() -> None:
    ...
    assert markdown == r"Observed that $\mathrm{Ph(i-PrO)SiH_2}$ (6) was consumed faster than other silanes."
...
def test_direct_typst_passthrough_keeps_existing_inline_math_latex_shape() -> None:
    markdown = build_direct_typst_passthrough_text(
        r"$ \mathbf{f}_{\alpha}^{IJ}(\mathbf{R}) $ is the key to understanding the limitations of the single-state Born-Oppenheimer approximation."
    )
    assert markdown.startswith(r"$\mathbf{f}_{\alpha}^{IJ}(\mathbf{R})$ is the key to understanding")
...
def test_direct_typst_sanitizer_keeps_only_inline_math_compat_cleanup() -> None:
    markdown = sanitize_direct_typst_inline_math(r"Body text $\mathscr{P}$ and $\angleABC$ maintain")
    assert markdown == r"Body text $\mathcal{P}$ and $\angle ABC$ maintain"


def test_direct_typst_boundary_module_matches_legacy_passthrough_behavior() -> None:
    text = r"Using 6-310** basis set，and keeping $E=mc^2$ and $\mathscr{P}$ unchanged."
...
def test_typst_markdown_renders_superscript_citation_as_text() -> None:
    ...
    markdown = build_markdown_from_parts("Method<f1-17a/> promotes", formula_map)
    assert markdown == "Method⁶ᶜ promotes"
...
def test_typst_markdown_promotes_left_right_bracket_formula() -> None:
    markdown = build_markdown_from_parts(r"Ions are \left[ NTf _ { 2 } \right] and ligands.", [])
    assert r"$\left[ NTf" in markdown
    assert r"\right]$" in markdown


def test_typst_markdown_promotes_bracketed_ion_pair() -> None:
    markdown = build_markdown_from_parts(r"Solvent uses [BMM][PF6] system.", [])
...
def test_promote_inline_math_like_text_for_garbled_reconstruction_blocks() -> None:
    text = (
        "The transition matrix Q_t indicates that with probability 1-β_t, x_t remains unchanged;"
        "each entry [Q_t]_ij represents the transition probability from state i to j, and x_t in {0,1}^K."
    )
...
def test_export_translation_template_direct_typst_keeps_raw_source_text() -> None:
    item = TextItem(
        ...
        text=r"Given the above considerations, CBFZ(\mathrm{CaO}_2) was used in the experiment.",
        segments=[
            {"type": "text", "content": "Given the above considerations, CBFZ"},
            {"type": "inline_equation", "content": r"(\mathrm{CaO}_2)"},
            {"type": "text", "content": " was used in the experiment."},
        ],
        lines=[],
