...
pub(super) async fn download_and_unpack_after_success(
    ...
    job.stage_detail = Some("OCR provider results ready, downloading original bundle".to_string());
    ...
    if !bundle_ready {
        ...
        job.stage = Some("translation_prepare".to_string());
        job.stage_detail =
            Some("OCR provider bundle reachability probe unstable, continuing with actual download fallback".to_string());
        job.updated_at = crate::models::now_iso();
        save_ocr_job(deps, job, parent_job_id).await?;
    }
            ...
            Err(err) => {
                ...
                if should_fallback_to_direct_download(&err, attempt, elapsed_secs, timeout_secs) {
                    ...
                    job.stage_detail = Some(
                        "OCR provider bundle probe repeatedly failed, switching to direct download with retry fallback"
                            .to_string(),
                    );
                    job.updated_at = crate::models::now_iso();
                ...
                job.stage_detail = Some(format!(
                    "OCR provider returned done but bundle not ready, retrying in {delay_secs}s (attempt {attempt}/{MINERU_BUNDLE_READY_RETRY_LIMIT})"
                ));
                job.updated_at = crate::models::now_iso();
                register_job_retry(job);
                record_custom_runtime_event_with_resources(
                    ...
                    "warn",
                    "retry_scheduled",
                    "OCR provider bundle reachability wait entering retry",
                    Some(serde_json::json!({
                        "scope": "mineru_bundle_ready_wait",
                        "attempt": attempt,
                        "max_attempts": MINERU_BUNDLE_READY_RETRY_LIMIT,
    ...
    loop {
        match client.download_bundle(full_zip_url, dest_path).await {
            ...
            Err(err) => {
                ...
                job.stage = Some("translation_prepare".to_string());
                job.stage_detail = Some(format!(
                    "OCR provider bundle download error, retrying in {delay_secs}s (attempt {attempt}/{MINERU_BUNDLE_DOWNLOAD_RETRY_LIMIT})"
                ));
                job.updated_at = crate::models::now_iso();
                register_job_retry(job);
                record_custom_runtime_event_with_resources(
                    ...
                    "warn",
                    "retry_scheduled",
                    "OCR provider bundle download entering retry",
                    Some(serde_json::json!({
