...
async fn run_paddle_poll_loop(
    ...
    job.stage_detail = Some("Paddle task submitted, waiting for parsing".to_string());
...
fn attach_paddle_runtime_error(
    ...
    let info = OcrProviderErrorInfo {
        ...
        operator_hint: Some("Please investigate using job logs and Paddle task status".to_string()),
