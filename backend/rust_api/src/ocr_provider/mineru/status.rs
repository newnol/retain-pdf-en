...
fn stage_and_detail(raw_state: &str, state: &OcrTaskState) -> (&'static str, String) {
    match state {
        OcrTaskState::WaitingUpload => ("mineru_upload", "Waiting for file upload to MinerU".to_string()),
        OcrTaskState::Queued => (
            "mineru_processing",
            "MinerU task accepted, waiting in queue".to_string(),
        ),
        OcrTaskState::Running => ("mineru_processing", "MinerU parsing file".to_string()),
        OcrTaskState::Converting => ("mineru_processing", "MinerU converting file".to_string()),
        OcrTaskState::Succeeded => (
            "translation_prepare",
            "MinerU results ready, preparing for translation".to_string(),
        ),
        OcrTaskState::Failed => ("failed", "MinerU processing failed".to_string()),
        OcrTaskState::Unknown => (
            "mineru_processing",
            format!("MinerU status: {}", raw_state.trim()),
        ),
    }
}
