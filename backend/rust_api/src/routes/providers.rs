...
pub async fn validate_mineru_token(
        ...
        .await
    {
        Ok(result) => MineruTokenValidationView {
            ok: true,
            status: "valid",
            summary: "MinerU Token valid".to_string(),
            retryable: false,
...
pub async fn validate_paddle_token(
    ...
    let view = match client.probe_token().await {
        Ok(result) => MineruTokenValidationView {
            ok: true,
            status: "valid",
            summary: "Paddle Access Token valid".to_string(),
            retryable: false,
            provider_code: Some("0".to_string()),
            provider_message: Some("ok".to_string()),
            operator_hint: Some(
                "Authentication passed; using random task ID for auth-only probe, no real OCR task triggered".to_string(),
            ),
            trace_id: result.trace_id,
            base_url: client.base_url.clone(),
            checked_at,
...
async fn classify_deepseek_probe_response(
    ...
    if status_code.is_success() {
        return MineruTokenValidationView {
            ok: true,
            status: "valid",
            summary: "DeepSeek API Key valid".to_string(),
            retryable: false,
    ...
    let summary = if status == "unauthorized" {
        ...
        format!("DeepSeek API returned {}", status_code.as_u16())
    };
    ...
    if let Some(provider_err) = err.downcast_ref::<PaddleProviderError>() {
        ...
        if valid_404 {
            return MineruTokenValidationView {
                ...
                operator_hint: Some("Authentication passed; non-existent random probe task is expected".to_string()),
