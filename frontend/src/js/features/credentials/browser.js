...
export function mountBrowserCredentialsFeature({
  ...
  function setCredentialDialogMode(setupMode = false) {
    ...
    $("browser-credentials-title").textContent = setupMode ? "Initial Setup" : "API Settings";
    const subtitle = $("browser-credentials-subtitle");
    if (subtitle) {
      const text = setupMode
        ? "Please fill in OCR Provider credentials and DeepSeek Key. The desktop app will start the local service automatically when done."
        : "";
      subtitle.textContent = text;
      subtitle.classList.toggle("hidden", !text);
    }
    $("browser-credentials-save-btn").textContent = setupMode ? "Save & Launch" : "Save";
  ...
  async function runOcrTokenValidation(providerId, token, { showResult = true } = {}) {
    ...
    try {
      ...
      resetOcrValidationCache();
      if (showResult) {
        setOcrValidationMessage(`${definition.label} Token check failed, please try again later.`, "error", definition.id);
      }
      return {
        ok: false,
