...
async function fetchProtectedBytes(fetchProtected, url, label) {
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`Failed to read ${label}: ${resp.status} ${text || "unknown error"}`);
  }
...
export function mountReaderDialogFeature({
  ...
  function setLoadingProgress(percent = 0, text = "Preparing bilingual reader...") {
  ...
  async function handleMergedDownload() {
    ...
    try {
      const [sourceBytes, translatedBytes] = await Promise.all([
        fetchProtectedBytes(fetchProtected, sourcePdf, "Source PDF"),
        fetchProtectedBytes(fetchProtected, translatedPdf, "Translated PDF"),
      ]);
  ...
  function open(input) {
    ...
    if (frame) {
      setLoading(true);
      setLoadingProgress(8, "Preparing bilingual reader...");
      const component = readerDialogComponent();
  ...
  function close() {
    ...
    setLoading(false);
    setLoadingProgress(0, "Preparing bilingual reader...");
