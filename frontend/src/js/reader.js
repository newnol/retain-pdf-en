...
const progressCopy = {
  boot: "Preparing bilingual reader...",
  metadata: "Reading task info...",
  both: "Loading source PDF and translated PDF...",
  sourceOnly: "Source PDF loaded, loading translated PDF...",
  translatedOnly: "Translated PDF loaded, loading source PDF...",
  ready: "Bilingual reader is ready",
  failed: "Bilingual reader failed to load",
};
...
async function loadPdfDocument(itemOrUrl, label) {
  ...
  if (!resp.ok) {
    throw new Error(`Failed to read ${label}。(${resp.status})`);
  }
...
async function initializeReader() {
  ...
  try {
    ...
    const [sourceResult, translatedResult] = await Promise.allSettled([
      mountPdfViewer("reader-pdf", sourcePdf, "Source PDF", "reader-pdf-empty").finally(() => {
        progressState.sourceDone = true;
        syncReaderBootProgress();
      }),
      mountPdfViewer("reader-translated-pdf", translatedPdfUrl, "Translated PDF", "reader-translation-empty").finally(() => {
