class StatusTaskToolbar extends HTMLElement {
  connectedCallback() {
    ...
    if (variant === "downloads") {
      ...
      this.innerHTML = `
        <button id="status-detail-btn" type="button" class="task-toolbar-btn task-toolbar-btn-compact secondary" aria-label="Job Details" title="Job Details">
        ...
        <a id="reader-btn" class="button-link secondary disabled task-toolbar-btn hidden" href="#" aria-label="Bilingual Reader" title="Bilingual Reader" aria-disabled="true">
          <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
        ...
        <a id="pdf-btn" class="button-link disabled task-toolbar-btn task-toolbar-btn-primary hidden" href="#" target="_blank" rel="noopener noreferrer" aria-label="Download PDF" title="Download PDF">
          <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    this.innerHTML = `
      <button id="cancel-btn" type="button" class="task-toolbar-btn secondary" aria-label="Cancel Task" title="Cancel Task" disabled>
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M8.4 8.4l7.2 7.2M15.6 8.4l-7.2 7.2" stroke="currentColor" stroke-width="1.65" stroke-linecap="round"/>
        </svg>
        <span>Cancel</span>
      </button>
      <button id="stop-btn" type="button" class="task-toolbar-btn secondary" aria-label="Stop Polling" title="Stop Polling">
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <rect x="7.7" y="7.7" width="8.6" height="8.6" rx="2.2" fill="currentColor"/>
        </svg>
        <span>Stop</span>
      </button>
      <button id="back-home-btn" type="button" class="task-toolbar-btn secondary hidden" aria-label="Return to Home" title="Return to Home">
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
