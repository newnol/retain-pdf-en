...
function assertGroupedJobPayload(payload) {
  if (!isObject(payload)) {
    throw new Error("Submission failed: /api/v1/jobs requires a JSON object request body.");
  }
  if (!payload.workflow || !isObject(payload.source)) {
    throw new Error("Submission failed: /api/v1/jobs must use grouped JSON with at least workflow and source.");
  }
  ...
  if (leakedLegacyFields.length > 0) {
    throw new Error(
      `Submission failed: /api/v1/jobs no longer accepts legacy flat fields, found ${leakedLegacyFields.join(", ")}. Please use source/ocr/translation/render/runtime grouped structure.`,
    );
  }
}
...
export async function fetchJobPayload(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    if (resp.status === 404) {
      throw new Error("Task not found, please check if the job_id is correct.");
    }
    throw new Error(`Failed to read task, please try again later.(${resp.status})`);
  }
...
export async function fetchJobEvents(jobId, apiPrefix, limit = 50, offset = 0) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Failed to read event stream, please try again later.(${resp.status})`);
  }
  const payloadJson = await resp.json();
  return unwrapEnvelope(payloadJson);
}
...
export async function fetchJobArtifactsManifest(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Failed to read artifacts manifest, please try again later.(${resp.status})`);
  }
  const payloadJson = await resp.json();
  return unwrapEnvelope(payloadJson);
}
...
export async function fetchJobMarkdown(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Failed to read Markdown, please try again later.(${resp.status})`);
  }
  const payloadJson = await resp.json();
  return unwrapEnvelope(payloadJson);
}
...
export async function fetchTranslationDiagnostics(jobId, apiPrefix) {
  ...
  if (!resp.ok) {
    if (resp.status === 404) {
      throw new Error("Translation debug info not found, please confirm the task has completed translation.");
    }
    throw new Error(`Failed to read translation debug summary, please try again later.(${resp.status})`);
  }
  const payloadJson = await resp.json();
  return unwrapEnvelope(payloadJson);
}
...
export async function fetchTranslationItems(
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Failed to read translation debug list, please try again later.(${resp.status})`);
  }
  const payloadJson = await resp.json();
  return unwrapEnvelope(payloadJson);
}
...
export async function fetchTranslationItem(jobId, itemId, apiPrefix) {
  ...
  if (!resp.ok) {
    if (resp.status === 404) {
      throw new Error("Translation item not found, please check if item_id is correct.");
    }
    throw new Error(`Failed to read translation item details, please try again later.(${resp.status})`);
  }
  const payloadJson = await resp.json();
  return unwrapEnvelope(payloadJson);
}
...
export async function replayTranslationItem(jobId, itemId, apiPrefix) {
  ...
  if (!resp.ok) {
    ...
    if (resp.status === 404) {
      throw new Error("Translation item not found, cannot replay.");
    }
    if (contentType.includes("application/json")) {
      const errorPayload = await resp.json();
      throw new Error(`Replay translation item failed: ${errorPayload.message || JSON.stringify(errorPayload)}`);
    }
    const text = await resp.text();
    throw new Error(`Replay translation item failed: ${resp.status} ${text}`);
  }
  const payloadJson = await resp.json();
  return unwrapEnvelope(payloadJson);
}
...
export async function fetchJobList(
  ...
  if (!resp.ok) {
    throw new Error(`Failed to read recent jobs, please try again later.(${resp.status})`);
  }
  const payloadJson = await resp.json();
  return unwrapEnvelope(payloadJson);
}
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.addEventListener("load", () => {
      ...
      const message = typeof xhr.response === "object" && xhr.response
        ...
        : (xhr.responseText || "");
      reject(new Error(`Submission failed: ${xhr.status} ${message}`));
    });

    xhr.addEventListener("error", () => {
      reject(new Error(`Submission failed: Network error. Current API Base is ${apiBase()}, upload URL is ${url}. Please confirm the local service is running.`));
    });
