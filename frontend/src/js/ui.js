...
function stageIconMarkup(status, stageText) {
  ...
  if (text.includes("pars") || text.includes("ocr")) {
...
function updateRing(job) {
  ...
  const ringValueText = stageText || "Preparing";
...
function updateDetailDialog(job) {
  ...
  const note = job.status === "failed"
    ? "View failure reasons, suggestions, and event stream"
...
function renderFailureDetails(job) {
  ...
  details.retryable = typeof retryable === "boolean" ? (retryable ? "Yes" : "No") : "-";
  const component = document.querySelector("status-detail-dialog");
  ...
  if (history.length === 0) {
    ...
    if (component?.renderStageHistory) {
      component.renderStageHistory({
        markup: "",
        emptyText: "Backend did not return runtime.stage_history",
        hasItems: false,
      });
      return;
    }
    ...
    empty.textContent = "Backend did not return runtime.stage_history";
...
function renderEvents(eventsPayload) {
  ...
  status.textContent = `Latest ${items.length} items`;
  empty.classList.add("hidden");
  list.classList.remove("hidden");
  list.innerHTML = markup;
}
...
export function setUploadProgress(loaded, total) {
  ...
  if (hasNumbers) {
    ...
    $("upload-progress-text").textContent = `Uploading ${percent.toFixed(0)}%`;
    return;
  }
  if (fill) {
    fill.style.width = "18%";
...
export function resetUploadedFile() {
  ...
  $("upload-status").textContent = "No file uploaded";
