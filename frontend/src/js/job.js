...
export function summarizeStatus(status) {
  switch (status) {
    case "queued":
      return "Task submitted, waiting for backend to start processing.";
    case "running":
      return "Task is processing, please wait for the current stage to complete.";
    case "succeeded":
      return "Task completed, results are ready for download.";
    case "canceled":
      return "Task has been canceled.";
    case "failed":
      return "Task failed, please check the error message and retry.";
    default:
...
export function summarizePublicError(payload) {
  ...
  if (payload.status === "failed") {
    ...
    return detail || "Task failed. Please check your input files and configuration, then retry.";
  }
  if (payload.error) {
