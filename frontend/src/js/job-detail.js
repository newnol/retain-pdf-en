...
function summarizeMathMode(job) {
  ...
  if (mathMode === "direct_typst") {
    return "direct_typst - direct model formula output";
  }
  return mathMode || "-";
}
...
function summarizeArtifactLabel(key) {
  switch (`${key || ""}`.trim()) {
    ...
    case "typst_render_pdf":
      return "Typst Rendered PDF";
    case "markdown_raw":
      return "Markdown Raw";
    case "markdown_images_dir":
      return "Markdown Images Directory";
...
function renderArtifactsManifest(manifestPayload) {
  ...
  summary.textContent = items.length > 0 ? `${items.length} item(s)` : "No registered artifacts";
  if (items.length === 0) {
    container.innerHTML = '<div class="detail-empty">No artifacts manifest available</div>';
    return;
  }
...
function renderMarkdownContract(job, markdownPayload = null) {
  ...
  if (!contract.ready) {
    ...
    setText("detail-markdown-status", "No published Markdown for current task");
  ...
  if (!markdownPayload) {
    setText("detail-markdown-status", "Published, reading content...");
    return;
  }
  ...
  const statusBits = ["Loaded /markdown JSON"];
  if (fileName) {
...
async function renderMarkdownImagePreview(markdownPayload, imagesBaseUrl) {
  ...
  const previews = await Promise.all(previewRefs.map(async (ref) => {
    ...
    if (!absoluteUrl) {
      return { ref, absoluteUrl: "", objectUrl: "", error: "Cannot resolve image URL" };
    }
    try {
      ...
      return { ref, absoluteUrl, objectUrl: "", error: error.message || "Image loading failed" };
    }
  }));
  grid.innerHTML = previews.map((item) => `
    <article class="detail-markdown-image-card">
      ...
      ${item.objectUrl
        ...
        : `<div class="detail-empty">${escapeHtml(item.error || "Image unavailable")}</div>`}
...
function renderStageHistory(job) {
  ...
  list.innerHTML = history.map((entry, index) => {
    ...
    const exitAt = entry?.exit_at ? formatEventTimestamp(entry.exit_at) : (isTerminalStatus(job.status) ? "-" : "In Progress");
...
function renderEvents(eventsPayload) {
  ...
  status.textContent = items.length > 0 ? `All Events · ${items.length} items` : "All Events";
  if (items.length === 0) {
    list.innerHTML = "";
    list.classList.add("hidden");
    empty.classList.remove("hidden");
...
async function ensureEventsLoaded() {
  if (detailPageState.eventsPayload) {
    setEventsStatus(`All Events · ${Array.isArray(detailPageState.eventsPayload.items) ? detailPageState.eventsPayload.items.length : 0} items`);
  ...
  if (!detailPageState.eventsLoadingPromise) {
    ...
    detailPageState.eventsLoadingPromise = fetchAllJobEvents(detailPageState.job.job_id)
      ...
      .catch((error) => {
        setEventsStatus(error.message || "Failed to load event stream.");
        throw error;
      })
      .finally(() => {
...
function bindEventsLauncher() {
  $("detail-open-events-btn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      $("detail-open-events-btn").textContent = "View";
    } catch (_error) {
...
async function initializePage() {
  ...
  if (!jobId) {
    setText("detail-head-note", "Missing job_id. Open via detail.html?job_id=...");
    return;
  }
  ...
  setText("detail-head-note", isMockMode()
    ...
    : "This detail page can be shared directly via URL.");
  ...
  setEventsStatus("Not yet loaded");
  ...
  try {
    ...
    if (markdownPayload) {
      ...
      setText("detail-markdown-status", "Markdown marked as ready, but /markdown returned no content yet");
    }
  } catch (error) {
    renderMarkdownContract(job, null);
