...
async function renderPdfPage() {
  ...
  await new Promise((resolve, reject) => {
    ...
    els.pdfImage.onerror = () => reject(new Error(`Failed to load PDF page image: ${src}`));
...
function loadPayload(payload) {
  if (!payload || !Array.isArray(payload.samples) || payload.samples.length === 0) {
    setStatus("Sample JSON is invalid.", true);
    return;
  }
...
function applySample(sampleId) {
  ...
  if (typst?.fontSizePt && typst?.lineHeightRatio) {
    setStatus(
      `Loaded ${sample.block_id}，defaulting to Typst parameter seed：Font size ${typst.fontSizePt}pt，Line Height Ratio ${typst.lineHeightRatio}；Text still comes from translation JSON.`
    );
  } else if (best) {
    setStatus(
      `Loaded ${sample.block_id}，defaulting to Pretext Auto Fit：Font size ${best.fontSizePt}pt，Line Height Ratio ${best.lineHeightRatio}，Pretext Height ${best.height}pt.`
    );
  }
}
...
function compareNow() {
  ...
  try {
    ...
    setStatus(`Completed ${state.sample.block_id}  DOM / Pretext comparison.`);
    if (state.payload) {
      renderPdfPage().catch(() => {});
    }
    Promise.all([
...
function autoFit() {
  ...
  try {
    ...
    compareNow();
    setStatus(
      `Auto Fit completed: font size ${best.fontSizePt}pt, line height ratio ${best.lineHeightRatio}, pretext height ${best.height}pt, target ${targetHeightPt}pt, line count ${best.lineCount}, score ${round(best.score)}.`
    );
  } catch (error) {
...
async function loadSamplesForJob(jobId = null) {
  ...
  try {
    ...
    setStatus(`Sample loading failed：${error.message}`, true);
  }
}
...
function bindEvents() {
  ...
  els.fileInput.addEventListener("change", async (event) => {
    ...
    try {
      ...
      setStatus("Failed to read sample JSON.", true);
    }
  });
}
